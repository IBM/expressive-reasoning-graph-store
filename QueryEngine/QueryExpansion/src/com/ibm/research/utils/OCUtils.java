...
public class OCUtils {
  ...
  public static List<File> createRandomAboxAxiomPartitionsAsFiles(OWLOntology ont,
      int numberOfPartitions, long seed) throws OWLOntologyCreationException, IOException {
  ...
  public static OWLOntology createOWLThingTypeDeclForIndividuals(OWLOntology ont)
      throws OWLOntologyCreationException, IOException {
    OWLOntology ret = ont.getOntologyID() != null && ont.getOntologyID().getOntologyIRI() != null
        ? ont.getOWLOntologyManager().createOntology(ont.getOntologyID().getOntologyIRI().get())
  ...
  public static List<OWLOntology> createRandomAboxAxiomPartitions(OWLOntology ont,
      int numberOfPartitions, long seed) throws OWLOntologyCreationException {
    ...
    for (OWLAxiom ax : ont.getAxioms()) {
      if (OCUtils.isAboxAxiom(ax)) {
         ...
         * ret.get(numberOfPartitions-1).getOWLOntologyManager().addAxiom(ret.get(numberOfPartitions
         * -1), ax); } else {
         */
  ...
  public static void saveRDFXML(OWLOntology ont, File out, OWLDocumentFormat format)
      throws IOException {
    StringWriter sw = new StringWriter();
    if (format == null) {
      format = ont.getOWLOntologyManager().getOntologyFormat(ont);
    }
    PrintWriter bufw = new PrintWriter(sw);// new FileWriter(newKBFile));
    if (ont.getOWLOntologyManager().getOntologyFormat(ont) instanceof RDFDocumentFormat) {
      ((RDFDocumentFormat) ont.getOWLOntologyManager().getOntologyFormat(ont))
          .setAddMissingTypes(false);
    }
    RDFXMLRenderer render = new RDFXMLRenderer(ont, bufw, format);
    render.render();
    bufw.flush();
    String text = sw.toString();
    text = text.replace("NamedIndividual", "Thing");
    bufw.close();
    bufw = new PrintWriter(new FileWriter(out));
    bufw.write(text);
    bufw.flush();
    bufw.close();
  }
  ...
  public static Map<String, String> getPrefix2NamespaceMap(File owlFile)
    ...
    return ret;
    /*
     * OWLOntology ont = load(owlFile); return new HashMap<String,
     * String>(ont.getOWLOntologyManager()
