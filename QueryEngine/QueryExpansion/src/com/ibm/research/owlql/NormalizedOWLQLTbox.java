/******************************************************************************
 * Copyright (c) 2015 IBM Corporation. All rights reserved. This program and the accompanying
...
public class NormalizedOWLQLTbox {
  ...
  /**
   * mapping from key (ie, named property or concept) positive concept inclusion axioms. used as an
   * index to speed up the search for axioms to consider during rewrite.
   */
  protected Map<String, Set<OWLSubClassOfAxiom>> iri2PosIncAxInTbox;
  /**
   * mapping from key (ie, named property or concept) positive role inclusion axioms. used as an
   * index to speed up the search for axioms to consider during rewrite.
   */
  ...
  public NormalizedOWLQLTbox(NormalizedOWLQLTbox tbox) {
    ...
    try {
      ...
      this.normalizedOntology.getOWLOntologyManager().addAxioms(this.normalizedOntology,
          tbox.normalizedOntology.getAxioms());
      ...
      this.negObjectSubPropAxInTbox =
          new HashSet<OWLDisjointObjectPropertiesAxiom>(tbox.negObjectSubPropAxInTbox);
      this.negDataSubPropAxInTbox =
          new HashSet<OWLDisjointDataPropertiesAxiom>(tbox.negDataSubPropAxInTbox);
      ...
      this.negIncAxInNegClos =
          tbox.negIncAxInNegClos != null ? new HashSet<OWLSubClassOfAxiom>(tbox.negIncAxInNegClos)
              : null;
      this.negObjectSubPropAxInNegClos = tbox.negObjectSubPropAxInNegClos != null
          ? new HashSet<OWLDisjointObjectPropertiesAxiom>(tbox.negObjectSubPropAxInNegClos)
          : null;
      this.negDataSubPropAxInNegClos = tbox.negDataSubPropAxInNegClos != null
          ? new HashSet<OWLDisjointDataPropertiesAxiom>(tbox.negDataSubPropAxInNegClos)
          : null;
  ...
  public Set<OWLSubClassOfAxiom> getNegativeInclInNegClosure() {
    if (negIncAxInNegClos == null) {
      ...
      organizeNegativeClosure(negativeInclClosure, negIncAxInNegClos, negObjectSubPropAxInNegClos,
          negDataSubPropAxInNegClos);
    }
  ...
  protected Set<OWLAxiom> computeNegativeInclusionClosure(Set<OWLSubClassOfAxiom> deltaNegIncAx,
      Set<OWLDisjointObjectPropertiesAxiom> deltaNegObjSubPropAx,
      Set<OWLDisjointDataPropertiesAxiom> deltaNegDataSubPropAx,
      Set<OWLSubClassOfAxiom> deltaPosIncAx, Set<OWLSubPropertyAxiom> deltaPosSubPropAx) {

    return computeNegativeInclusionClosure(deltaNegIncAx, deltaNegObjSubPropAx,
        deltaNegDataSubPropAx, deltaPosIncAx, deltaPosSubPropAx, null);
  }


  protected Set<OWLAxiom> computeNegativeInclusionClosure(
      Collection<OWLSubClassOfAxiom> deltaNegIncAx,
      Collection<OWLDisjointObjectPropertiesAxiom> deltaNegObjSubPropAx,
      Collection<OWLDisjointDataPropertiesAxiom> deltaNegDataSubPropAx,
      Collection<OWLSubClassOfAxiom> deltaPosIncAx,
      Collection<OWLSubPropertyAxiom> deltaPosSubPropAx, OWLAxiom inconsistencyWitness) {
    Set<OWLAxiom> ret = new HashSet<OWLAxiom>();
    /*
     * if (negativeInclClosure!=null) { deltaNegIncAx.removeAll(negativeInclClosure);
     * deltaNegDataSubPropAx.removeAll(negativeInclClosure);
     * deltaNegObjSubPropAx.removeAll(negativeInclClosure); ret.addAll(negativeInclClosure);
     * deltaPosIncAx.removeAll(posIncAxInTbox); deltaPosSubPropAx.removeAll(posSubPropAxInTbox); }
     */
    ...
    for (int i = 0; i < 2; i++) {
      if (i == 0) {
        ...
        negIncAx = /* Collections.unmodifiableSet */new LinkedList<OWLSubClassOfAxiom>(
            getNegativeInclInNegClosure());// new
                                           // HashSet<OWLSubClassOfAxiom>(getNegativeInclInNegClosure());
        // negIncAx.addAll(deltaNegIncAx);
        negObjSubPropAx =
            /* Collections.unmodifiableSet */new LinkedList<OWLDisjointObjectPropertiesAxiom>(
                getNegativeObjectSubPropInNegClosure());// new
                                                        // HashSet<OWLDisjointObjectPropertiesAxiom>(getNegativeObjectSubPropInNegClosure());
        // negObjSubPropAx.addAll(deltaNegObjSubPropAx);
        negDataSubPropAx =
            /* Collections.unmodifiableSet */new LinkedList<OWLDisjointDataPropertiesAxiom>(
                getNegativeDataSubPropAxInClosure());// new
                                                     // HashSet<OWLDisjointDataPropertiesAxiom>(getNegativeDataSubPropAxInClosure());
      ...
      Set<OWLDisjointObjectPropertiesAxiom> newNegObjSubPropAx =
          new HashSet<OWLDisjointObjectPropertiesAxiom>();
      Set<OWLDisjointDataPropertiesAxiom> newNegDataSubPropAx =
          new HashSet<OWLDisjointDataPropertiesAxiom>();

      while (!negIncAx.isEmpty() || !negObjSubPropAx.isEmpty() || !negDataSubPropAx.isEmpty()) {
            ...
            for (OWLSubClassOfAxiom negSubAx : negIncAx) {
              ...
              if (isNegatedOWLQLRHSConcept(supclass)) {
                if (b2.equals(subclass)) {
                  newNegIncAx.add(fac.getOWLSubClassOfAxiom(b1, supclass));
                } else {
                  OWLClassExpression notSupclass = fac.getOWLObjectComplementOf(supclass).getNNF();
                  if (b2.equals(notSupclass)) {
                    OWLClassExpression notSubclass =
                        fac.getOWLObjectComplementOf(subclass).getNNF();
                    newNegIncAx.add(fac.getOWLSubClassOfAxiom(b1, notSubclass));
                  }
                }
              }
            }
        ...
        for (OWLSubPropertyAxiom posSubAx : posSubPropAx) {
          ...
          if (r1.isObjectPropertyExpression()) {
            b1 = fac.getOWLObjectSomeValuesFrom((OWLObjectPropertyExpression) r1,
                fac.getOWLThing());
            OWLObjectPropertyExpression r1Inv =
                fac.getOWLObjectInverseOf((OWLObjectProperty) r1).getSimplified();
            b1Inv = fac.getOWLObjectSomeValuesFrom(r1Inv, fac.getOWLThing());
          } else {
            assert r1.isDataPropertyExpression() : r1;
            b1 = fac.getOWLDataSomeValuesFrom((OWLDataPropertyExpression) r1, fac.getTopDatatype());
            b1Inv = null;
          }
          // if sub(R1, R2) in Tbox and sub(some(R2, top), not(B)) or sub(B, not(some(R2, top))) is
          // in negClos, then
          // add sub(some(R1, top), not(B)) in negClos;

          // if sub(R1, R2) in Tbox and sub(some(inv(R2), top), not(B)) or sub(B, not(some(inv(R2),
          // top))) is in negClos, then
          ...
          for (OWLSubClassOfAxiom negSubAx : negIncAx) {
            ...
            if (isNegatedOWLQLRHSConcept(supclass)) {
              if (subclass.getClassExpressionType()
                  .equals(ClassExpressionType.OBJECT_SOME_VALUES_FROM)
                ...
                if (r2.equals(prop)) {
                  ...
                  OWLObjectPropertyExpression propInv =
                      fac.getOWLObjectInverseOf(oprop).getSimplified();
                  if (r2.equals(propInv)) {
                    assert b1Inv != null : r2 + "\n" + prop + "\n" + propInv;
                    newNegIncAx.add(fac.getOWLSubClassOfAxiom(b1Inv, supclass));
                  }
                }
              }
              OWLClassExpression notSupclass = fac.getOWLObjectComplementOf(supclass).getNNF();
              if (notSupclass.getClassExpressionType()
                  .equals(ClassExpressionType.OBJECT_SOME_VALUES_FROM)
                  || notSupclass.getClassExpressionType()
                      .equals(ClassExpressionType.DATA_SOME_VALUES_FROM)) {
                OWLQuantifiedRestriction some = (OWLQuantifiedRestriction) notSupclass;
                OWLPropertyExpression prop = some.getProperty();
                if (r2.equals(prop)) {
                  OWLClassExpression notSubclass = fac.getOWLObjectComplementOf(subclass).getNNF();
                  newNegIncAx.add(fac.getOWLSubClassOfAxiom(b1, notSubclass));
                } else if (prop.isOWLObjectProperty()) {
                  OWLObjectProperty oprop = (OWLObjectProperty) prop;
                  OWLObjectPropertyExpression propInv =
                  ...
                  if (r2.equals(propInv)) {
                    ...
                    OWLClassExpression notSubclass =
                        fac.getOWLObjectComplementOf(subclass).getNNF();
                    newNegIncAx.add(fac.getOWLSubClassOfAxiom(b1Inv, notSubclass));
                  }
                }
              }
            }
          ...
          Set<Set<? extends OWLPropertyExpression>> disjointProperties =
              new HashSet<Set<? extends OWLPropertyExpression>>();
          ...
          for (Set<? extends OWLPropertyExpression> props : disjointProperties) {
            ...
            if (props.contains(r2)) {
              if (r2.isObjectPropertyExpression()) {
                ...
                disjProps.addAll((Set<OWLObjectPropertyExpression>) props);
                if (props.size() > 1) {
                  disjProps.remove(r2);
                }
                newNegObjSubPropAx.add(fac.getOWLDisjointObjectPropertiesAxiom(disjProps));
              } else {
                ...
                disjProps.addAll((Set<OWLDataPropertyExpression>) props);
                if (props.size() > 1) {
                  disjProps.remove(r2);
                }
                newNegDataSubPropAx.add(fac.getOWLDisjointDataPropertiesAxiom(disjProps));
              }
            }
            ...
            if (r2.isOWLObjectProperty()) {
              OWLObjectProperty or2 = (OWLObjectProperty) r2;
              OWLObjectPropertyExpression invR2 = fac.getOWLObjectInverseOf(or2).getSimplified();
              if (props.contains(invR2)) {
                Set<OWLObjectPropertyExpression> disjProps =
                    new HashSet<OWLObjectPropertyExpression>();
                disjProps.add(fac.getOWLObjectInverseOf((OWLObjectProperty) r1).getSimplified());
                disjProps.addAll((Set<OWLObjectPropertyExpression>) props);
                if (props.size() > 1) {
        ...
        // if sub(some(R), not(some(R)) or sub(some(inv(R)), not(some(inv(R))) or sub(R, not(R)) in
        // negClos, then
        ...
        for (OWLSubClassOfAxiom ax : negIncAx) {
          ...
          OWLClassExpression negSupclass = fac.getOWLObjectComplementOf(supclass).getNNF();
          if (negSupclass.equals(subclass)) {
            if (subclass.getClassExpressionType()
              ...
              OWLObjectPropertyExpression rInv =
                  fac.getOWLObjectInverseOf((OWLObjectProperty) r).getSimplified();
              OWLObjectSomeValuesFrom someInv =
                  fac.getOWLObjectSomeValuesFrom(rInv, fac.getOWLThing());
              OWLClassExpression negSomeInv = fac.getOWLObjectComplementOf(someInv).getNNF();
              newNegIncAx.add(fac.getOWLSubClassOfAxiom(someInv, negSomeInv));
              Set<OWLObjectPropertyExpression> disjProps =
                  new HashSet<OWLObjectPropertyExpression>();
              disjProps.add(r);
              newNegObjSubPropAx.add(fac.getOWLDisjointObjectPropertiesAxiom(disjProps));
        ...
        for (OWLDisjointObjectPropertiesAxiom ax : negObjSubPropAx) {
          ...
          if (props.size() == 1) {
            ...
            OWLObjectSomeValuesFrom some = fac.getOWLObjectSomeValuesFrom(r, fac.getOWLThing());
            OWLClassExpression negSome = fac.getOWLObjectComplementOf(some).getNNF();
            newNegIncAx.add(fac.getOWLSubClassOfAxiom(some, negSome));
        ...
        for (OWLDisjointDataPropertiesAxiom ax : negDataSubPropAx) {
          ...
          if (props.size() == 1) {
            ...
            OWLDataSomeValuesFrom some = fac.getOWLDataSomeValuesFrom(r, fac.getTopDatatype());
            OWLClassExpression negSome = fac.getOWLObjectComplementOf(some).getNNF();
            newNegIncAx.add(fac.getOWLSubClassOfAxiom(some, negSome));
          }
        }
  ...
  protected void init(Set<OWLAxiom> axioms, boolean normalize, boolean computeNegClos) {
    try {
      ...
      Set<OWLAxiom> normalizedAxioms =
          normalize ? normalizer.toQLNormalForm(axioms, nonOWLQLAxioms) : axioms;
      normalizedOntology = normalizedOntology != null ? normalizedOntology
      ...
      for (OWLObjectProperty p : originalOnt.getObjectPropertiesInSignature()) {
        if (!normalizedOntology.getObjectPropertiesInSignature().contains(p)) {
          normalizedOntology.getOWLOntologyManager().addAxiom(normalizedOntology, normalizedOntology
              .getOWLOntologyManager().getOWLDataFactory().getOWLDeclarationAxiom(p));
        }
      }
      ...
      for (OWLClass c : originalOnt.getClassesInSignature()) {
        if (!normalizedOntology.getClassesInSignature().contains(c)) {
          normalizedOntology.getOWLOntologyManager().addAxiom(normalizedOntology, normalizedOntology
              .getOWLOntologyManager().getOWLDataFactory().getOWLDeclarationAxiom(c));
        }
      }
      //
      if (logger.isDebugEnabled()) {
      ...
      organizeTboxAxioms(normalizedAxioms, negIncAxInTbox, negObjectSubPropAxInTbox,
          negDataSubPropAxInTbox, posIncAxInTbox, posSubPropAxInTbox, reflexiveProps,
          irreflexiveProps, iri2PosIncAxInTbox, iri2PosSubPropAxInTbox);
      //
      if (computeNegClos) {
        negativeInclClosure = computeNegativeInclusionClosure(negIncAxInTbox,
            negObjectSubPropAxInTbox, negDataSubPropAxInTbox, posIncAxInTbox, posSubPropAxInTbox);
      }
    } catch (OWLOntologyCreationException e) {
      throw new RuntimeException(e);
    }
  ...
  public static String getKey(OWLClassExpression lhsConcept) {
    ...
    } else if (lhsConcept.getClassExpressionType()
        .equals(ClassExpressionType.OBJECT_SOME_VALUES_FROM)
  ...
  private static void organizeNegativeClosure(Collection<OWLAxiom> normalizedNegClosureAxioms,
      Collection<OWLSubClassOfAxiom> negIncAxInNegClos,
      Collection<OWLDisjointObjectPropertiesAxiom> negObjectSubPropAxInNegClos,
      Collection<OWLDisjointDataPropertiesAxiom> negDataSubPropAxInNegClos) {
    organizeTboxAxioms(normalizedNegClosureAxioms, negIncAxInNegClos, negObjectSubPropAxInNegClos,
        negDataSubPropAxInNegClos, new HashSet<OWLSubClassOfAxiom>(),
        new HashSet<OWLSubPropertyAxiom>(), new HashSet<OWLObjectProperty>(),
        new HashSet<OWLObjectProperty>(), new HashMap<String, Set<OWLSubClassOfAxiom>>(),
        new HashMap<String, Set<OWLSubPropertyAxiom>>());
  }

  private static void organizeTboxAxioms(Collection<OWLAxiom> normalizedAxioms,
      Collection<OWLSubClassOfAxiom> negIncAxInTbox,
      Collection<OWLDisjointObjectPropertiesAxiom> negObjectSubPropAxInTbox,
      Collection<OWLDisjointDataPropertiesAxiom> negDataSubPropAxInTbox,
      Collection<OWLSubClassOfAxiom> posIncAxInTbox,
      Collection<OWLSubPropertyAxiom> posSubPropAxInTbox,
      Collection<OWLObjectProperty> reflexivePropsInTbox,
      Collection<OWLObjectProperty> irreflexivePropsInTbox,
      Map<String, Set<OWLSubClassOfAxiom>> iri2PosIncAxInTbox,
      Map<String, Set<OWLSubPropertyAxiom>> iri2PosSubPropAxInTbox) {
    // organize Tbox axioms per type
    for (OWLAxiom ax : normalizedAxioms) {
      if (ax.getAxiomType().equals(AxiomType.SUBCLASS_OF)) {
        ...
        if (!superclass.isAnonymous()
          ...
          assert superclass.getClassExpressionType()
              .equals(ClassExpressionType.OBJECT_ALL_VALUES_FROM)
              || superclass.getClassExpressionType()
                  .equals(ClassExpressionType.DATA_ALL_VALUES_FROM)
              || superclass.getClassExpressionType()
                  .equals(ClassExpressionType.OBJECT_COMPLEMENT_OF) : ax;
  ...
  public Set<OWLObjectProperty> getReflexiveProperties() {
    if (reflexiveProps == null) {
      reflexiveProps = new HashSet<OWLObjectProperty>();
      for (OWLReflexiveObjectPropertyAxiom ax : normalizedOntology
  ...
  public Set<OWLObjectProperty> getIrreflexiveProperties() {
    if (irreflexiveProps == null) {
      irreflexiveProps = new HashSet<OWLObjectProperty>();
      for (OWLIrreflexiveObjectPropertyAxiom ax : normalizedOntology
  ...
  /*
   * public NormalizedOWLQLTbox add(Set<OWLAxiom> axioms) { NormalizedOWLQLTbox ret = copy();
   * Set<OWLAxiom> normalizedAxioms = ret.getNormalizer().toQLNormalForm(axioms,
   * ret.nonOWLQLAxioms); ret.init(normalizedAxioms, false, false); Set<OWLSubClassOfAxiom>
   * deltaNegIncAx = new HashSet<OWLSubClassOfAxiom>(); Set<OWLDisjointObjectPropertiesAxiom>
   * deltaNegObjSubPropAx = new HashSet<OWLDisjointObjectPropertiesAxiom>();
   * Set<OWLDisjointDataPropertiesAxiom> deltaNegDataSubPropAx = new
   * HashSet<OWLDisjointDataPropertiesAxiom>(); Set<OWLSubClassOfAxiom> deltaPosIncAx = new
   * HashSet<OWLSubClassOfAxiom>(); Set<OWLSubPropertyAxiom> deltaPosSubPropAx = new
   * HashSet<OWLSubPropertyAxiom>(); organizeTboxAxioms(normalizedAxioms, deltaNegIncAx,
   * deltaNegObjSubPropAx, deltaNegDataSubPropAx,deltaPosIncAx, deltaPosSubPropAx, new
   * HashMap<String, Set<OWLSubClassOfAxiom>>(), new HashMap<String, Set<OWLSubPropertyAxiom>>());
   * Set<OWLAxiom> newNegClos = ret.computeNegativeInclusionClosure(deltaNegIncAx,
      ...
      for (OWLDisjointObjectPropertiesAxiom ax : getNegativeObjectSubPropInNegClosure()) {
        if (ax.getProperties().size() == 1) {
          unsatisfiableProperties
              .add(ax.getProperties().iterator().next().getNamedProperty().asOWLObjectProperty());
        }
      }
    }
  ...
  /*
   * public boolean isUnsatisfiable(OWLClassExpression normalizedConcept) { if
   * (!normalizedConcept.isAnonymous()) { if (normalizedConcept.isOWLNothing()) { return true; } if
  ...
  public boolean isSubClass(OWLClassExpression sub, OWLClassExpression sup) {
    ...
    if (sup.getClassExpressionType().equals(ClassExpressionType.OBJECT_COMPLEMENT_OF)
        && (((OWLObjectComplementOf) sup).getOperand().isOWLNothing()
            || getUnsatisfiableNamedClasses()
                .contains(((OWLObjectComplementOf) sup).getOperand()))) {
      return true;
    }
    //
    ...
    if ((sub.getClassExpressionType().equals(ClassExpressionType.OBJECT_SOME_VALUES_FROM)
        ...
        && (((OWLQuantifiedRestriction) sub).getProperty().isOWLBottomObjectProperty()
            ...
            || getUnsatisfiableProperties()
                .contains(((OWLQuantifiedRestriction) sub).getProperty()))) {
      return true;
    }
    //
    ...
    Set<OWLAxiom> normalizedAxioms =
        ret.getNormalizer().toQLNormalForm(newAxioms, newNonOWLQLAxioms);
    ...
    List<OWLDisjointObjectPropertiesAxiom> deltaNegObjSubPropAx =
        new LinkedList<OWLDisjointObjectPropertiesAxiom>();
    List<OWLDisjointDataPropertiesAxiom> deltaNegDataSubPropAx =
        new LinkedList<OWLDisjointDataPropertiesAxiom>();
    ...
    organizeTboxAxioms(normalizedAxioms, deltaNegIncAx, deltaNegObjSubPropAx, deltaNegDataSubPropAx,
        deltaPosIncAx, deltaPosSubPropAx, new HashSet<OWLObjectProperty>(),
        new HashSet<OWLObjectProperty>(), new HashMap<String, Set<OWLSubClassOfAxiom>>(),
        new HashMap<String, Set<OWLSubPropertyAxiom>>());
    // inconsistent iff subclass(NC, not(NC)) is in the negative closure
    OWLAxiom inconsistencyWitness =
        fac.getOWLSubClassOfAxiom(newConcept, newConcept.getComplementNNF());
    Set<OWLAxiom> newNegClos =
        ret.computeNegativeInclusionClosure(deltaNegIncAx, deltaNegObjSubPropAx,
            deltaNegDataSubPropAx, deltaPosIncAx, deltaPosSubPropAx, inconsistencyWitness);
  ...
  public boolean isSubProperty(OWLPropertyExpression sub, OWLPropertyExpression sup) {
    ...
    if (sub.isDataPropertyExpression()) {
      ...
      newAxioms
          .add(fac.getOWLDisjointDataPropertiesAxiom(newProp, (OWLDataPropertyExpression) sup));
      ...
      newAxioms
          .add(fac.getOWLDisjointObjectPropertiesAxiom(newProp, (OWLObjectPropertyExpression) sup));
      // inconsistent iff subprop(NP, not(NP)) is in the negative closure
  ...
  public ConsistencyCheckResult computeConsistencyCheck(Taxonomy taxo) {
    ...
    for (OWLAxiom ax : getNegativeInclClosure()) {
      ...
      if (ax.getAxiomType().equals(AxiomType.SUBCLASS_OF)) {
        ...
        assert NormalizedOWLQLTbox
            .isNegatedOWLQLRHSConcept(superclass) : "Invalid negative subclass axiom: " + ax;
        Triple t1 = toTriple(subclass, var, varGen, getFactory());
        if (!getReflexiveProperties()
            .contains(getFactory().getOWLObjectProperty(IRI.create(t1.getPredicate().getURI())))) {
          triples.add(t1);
        }
        OWLClassExpression negSuperclass =
            getFactory().getOWLObjectComplementOf(superclass).getNNF();
        Triple t2 = toTriple(negSuperclass, var, varGen, getFactory());
        if (!getReflexiveProperties()
            .contains(getFactory().getOWLObjectProperty(IRI.create(t2.getPredicate().getURI())))) {
          triples.add(t2);
        }
        if (triples.isEmpty()) {
        ...
        if (numberOfReflexiveProps == props.size()) {
          ...
          // we add a new disjoint of the form: R(x, x), where R is the property which is not
          // reflexive
          {
   ...
   * @return
   */
  protected static Triple toTriple(OWLClassExpression lhsConcept, Node var,
      NewVariableGenerator varGen, OWLDataFactory fac) {
    if (!lhsConcept.isAnonymous()) {
      return new Triple(var, NodeFactory.createURI(RDFConstants.RDF_TYPE),
      ...
      OWLPropertyRange propRange = (OWLPropertyRange) rest.getFiller();
      if (prop.isObjectPropertyExpression()) {
        prop = ((OWLObjectPropertyExpression) prop).getSimplified();
      }
      assert propRange.equals(fac.getOWLThing())
          || propRange.equals(fac.getTopDatatype()) : propRange;
      if (!prop.isAnonymous()) {
        OWLProperty p = (OWLProperty) prop;
        return new Triple(var, NodeFactory.createURI(p.getIRI().toString()),
        ...
        return new Triple(NodeFactory.createVariable(varGen.createNewVariable()),
            NodeFactory.createURI(op.getIRI().toString()), var);

      }
    } else {
      throw new RuntimeException(lhsConcept
          + " is not a valid concept expression in the left hand side of a subclass axiom in OWL QL");
    }
  ...
  protected static Triple toTriple(OWLPropertyExpression pe, Node x, Node y,
    ...
    if (!pe.isAnonymous()) {
      ...
      return new Triple(x, NodeFactory.createURI(p.getIRI().toString()), y);
    } else {
      ...
      return new Triple(y, NodeFactory.createURI(p.getIRI().toString()), x);
    }
  }
