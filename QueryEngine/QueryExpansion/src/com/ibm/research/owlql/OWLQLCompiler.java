...
public class OWLQLCompiler implements IOWLQLCompiler {
  ...
  public OWLQLCompiler(OWLOntology originalOntTbox) {
    super();
    tbox =
        new NormalizedOWLQLTbox(NormalizedOWLQLTbox.getTboxRboxAxioms(originalOntTbox.getAxioms()));
  }
  ...
  protected Triple basicRewrite(Triple t, OWLSubClassOfAxiom ax, String key,
  ...
  protected Triple basicRewrite(Triple t, OWLSubPropertyAxiom ax, String key,
      NewVariableGenerator varGen) {
    ...
    if (!pred.getURI().equals(RDFConstants.RDF_TYPE)) {
      ...
      // same polarity?
      if ((sub.isAnonymous() && sup.isAnonymous()) || (!sub.isAnonymous() && !sup.isAnonymous())) {
  ...
  protected void removeQueryWithGeneratedTerms(Set<ConjunctiveQuery> q) {
    ...
    logger.debug("Number of removed rewritten queries: " + removed + " ("
        + (((float) removed) / q.size()) + ")%");
  ...
  protected Set<Triple> getRecursiveTriplesSubstitution(Triple t, NewVariableGenerator varGen,
    ...
    return computeRecursiveTriplesSubstitution(t, varGen, allowTriplesAbsentFromAbox,
        new HashSet<Triple>(), triple2Results);

  }

  protected Set<Triple> computeRecursiveTriplesSubstitution(Triple t, NewVariableGenerator varGen,
      boolean allowTriplesAbsentFromAbox, Set<Triple> alreadyFound,
      Map<Triple, Set<Triple>> triple2Results) {
    alreadyFound.add(t);
    ...
    for (Triple nt : ts) {
      if (!alreadyFound.contains(nt)) {
        results.addAll(computeRecursiveTriplesSubstitution(nt, varGen, allowTriplesAbsentFromAbox,
            alreadyFound, triple2Results));
      }
    }
      ...
      for (ConjunctiveQuery cq : prevNewCqs) {
        for (Triple t : new HashSet<Triple>(cq.getTriples())) {
          ...
          Set<String> unboundVars =
              new HashSet<String>(getValidNameForUnboundVariable(getAllVariables(cqTriples)));
          //
          Set<Triple> newTs = triple2TripleSubstitutions.get(t);
          if (newTs == null) {
            newTs = getRecursiveTriplesSubstitution(t, varGen, false, triple2TripleSubstitutions);// singleStepTriplesSubstitution(t,
                                                                                                  // varGen);//
  ...
  protected ConjunctiveQuery unify(Triple t1, Triple t2, ConjunctiveQuery q) {
    ...
    for (String var : q.getResultVars()) {
      ...
      ret.addFilter(new E_Equals(left, right));
    ...
    for (Triple triple : conjQuery.getTriples()) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        if (t.isVariable()) {
          ...
          if (unboundVars.contains(var) && !isValidNameForUnboundVariable(var)) {
  ...
  public static boolean isValidNameForUnboundVariable(String st) {
    if (st.startsWith(UNBOUND_VARIABLE_PREFIX) && st.length() > UNBOUND_VARIABLE_PREFIX.length()) {
  ...
  protected Triple renameTripleToAvoidUnboundVarCapture(Triple triple,
      Set<String> unboundVarsInConjQ, NewVariableGenerator varGen) {
