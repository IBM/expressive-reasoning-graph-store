...
public class TaxonomyImpl implements Taxonomy {
  ...
  public static TaxonomyBuilder<OWLClassExpression> buildClassHierarchy(NormalizedOWLQLTbox tbox,
      Collection<OWLClassExpression> additionalClassExpression) {
    ...
    TaxonomyBuilder<OWLClassExpression> taxoBuilder = new TaxonomyBuilder<OWLClassExpression>(
        classes, fac.getOWLThing(), fac.getOWLNothing(), subcomp);
    taxoBuilder.build();
    logger.debug("Number of direct subsumption tests performed: {}",
        subcomp.numOfDirectSubsumptionTests);
    long size = classes.size();
    logger.debug("Worst Case number of direct subsumption tests performed: {}^2 = {}", size,
        size * size);
    return taxoBuilder;
  }

  public static TaxonomyBuilder<OWLPropertyExpression> buildObjectPropertyHierarchy(
      NormalizedOWLQLTbox tbox,
      Collection<? extends OWLPropertyExpression> additionalPropertyExpressions) {
    OWLOntology ont = tbox.getNormalizedOntology();
    OWLDataFactory fac = ont.getOWLOntologyManager().getOWLDataFactory();
    ...
    TaxonomyBuilder<OWLPropertyExpression> taxoBuilder = new TaxonomyBuilder<OWLPropertyExpression>(
        props, fac.getOWLTopObjectProperty(), fac.getOWLBottomObjectProperty(), subcomp);
    taxoBuilder.build();
    logger.debug("Number of direct subsumption tests performed: {}",
        subcomp.numOfDirectSubsumptionTests);
  ...
  public static TaxonomyBuilder<OWLPropertyExpression> buildDataPropertyHierarchy(
      NormalizedOWLQLTbox tbox,
      Collection<? extends OWLPropertyExpression> additionalPropertyExpressions) {
    OWLOntology ont = tbox.getNormalizedOntology();
    OWLDataFactory fac = ont.getOWLOntologyManager().getOWLDataFactory();
    ...
    SubPropertyComputation subcomp = new SubPropertyComputation(tbox);
    TaxonomyBuilder<OWLPropertyExpression> taxoBuilder = new TaxonomyBuilder<OWLPropertyExpression>(
        props, fac.getOWLTopDataProperty(), fac.getOWLBottomDataProperty(), subcomp);
    taxoBuilder.build();
    logger.debug("Number of direct subsumption tests performed: {}",
        subcomp.numOfDirectSubsumptionTests);
    long size = props.size();
  ...
  public static void addImpliedTaxonomy(OWLOntology ont) {
    ...
    for (TaxonomyBuilder<OWLClassExpression>.TaxoNode node : classTaxo.getNodes()) {
      ...
      for (Iterator<? extends TaxonomyBuilder<OWLClassExpression>.TaxoNode> it = node.getSubs(); it
          .hasNext();) {
    ...
    for (TaxonomyBuilder<OWLPropertyExpression>.TaxoNode node : opTaxo.getNodes()) {
      ...
      assert rep != null : node;
      for (Iterator<? extends TaxonomyBuilder<OWLPropertyExpression>.TaxoNode> it =
          node.getSubs(); it.hasNext();) {
        ...
        for (OWLPropertyExpression subprop : subNode.getEquivalentElements()) {
          ont.getOWLOntologyManager().addAxiom(ont,
              fac.getOWLSubObjectPropertyOfAxiom((OWLObjectPropertyExpression) subprop, rep));
        }
      }
    ...
    for (TaxonomyBuilder<OWLPropertyExpression>.TaxoNode node : dpTaxo.getNodes()) {
      ...
      for (Iterator<? extends TaxonomyBuilder<OWLPropertyExpression>.TaxoNode> it =
        ...
        for (OWLPropertyExpression subprop : subNode.getEquivalentElements()) {
          ont.getOWLOntologyManager().addAxiom(ont,
              fac.getOWLSubDataPropertyOfAxiom((OWLDataPropertyExpression) subprop, rep));
        }
      }
  ...
  public TaxonomyImpl(NormalizedOWLQLTbox tbox) throws OWLOntologyCreationException {
    ...
    // Set<OWLDataPropertyExpression> additionalDataPropertyExpressions = new
    ...
    for (OWLProperty p : props) {
      if (p.isOWLTopDataProperty() || p.isOWLTopObjectProperty() || p.isOWLBottomDataProperty()
      ...
      if (p.isObjectPropertyExpression()) {
        existP = tbox.getFactory().getOWLObjectSomeValuesFrom((OWLObjectProperty) p,
            tbox.getFactory().getOWLThing());
      } else {
        existP = tbox.getFactory().getOWLDataSomeValuesFrom((OWLDataProperty) p,
            tbox.getFactory().getTopDatatype());
      }
      ...
      if (p.isObjectPropertyExpression()) {
        ...
        OWLClassExpression existInvP = tbox.getFactory().getOWLObjectSomeValuesFrom(
            op.getInverseProperty().getSimplified(), tbox.getFactory().getOWLThing());
        ...
        OWLObjectPropertyExpression invP =
            tbox.getFactory().getOWLObjectInverseOf(op).getSimplified();
        /*
         * for (OWLObjectPropertyExpression exp: op.getInverses(ont)) { if
         * (!exp.getSimplified().isAnonymous()) { inv = exp.getSimplified().asOWLObjectProperty();
         * ATermAppl invPAterm = ATermUtils.makeTermAppl(inv.getIRI().toString());
         * propExp2AtermInTaxon.put(invP,invPAterm); aterm2propExp.put(invPAterm, invP) break; } }
         */
        if (inv == null) {
  ...
  @Override
  public boolean isSubClass(OWLClassExpression normalizedSub, OWLClassExpression normalizedSup) {
  ...
  @Override
  public Set<OWLClassExpression> getMostGeneralSubsumees(Set<OWLClassExpression> classes,
    ...
    if (isDataRole) {
      for (OWLPropertyExpression candR : roleCandidates) {
        ...
        for (OWLPropertyExpression sub : roleCandidates) {
          ...
          if (dataPropTaxo.isSubsumedBy(candR, sub) && !dataPropTaxo.isSubsumedBy(sub, candR)) {
        ...
        if (acceptCandR) {
          ...
          ret.add(fac.getOWLDataSomeValuesFrom((OWLDataPropertyExpression) candR,
              fac.getTopDatatype()));
        }
      }
      ...
      for (OWLPropertyExpression candR : roleCandidates) {
        ...
        for (OWLPropertyExpression sub : roleCandidates) {
          ...
          if (isSubClass(someInvCandR, someInvSub) && !isSubClass(someInvSub, someInvCandR)) {
            // candR is a strict sub property of sub
            acceptCandR = false;
            break;
          }
        ...
        if (acceptCandR) {
          ...
          ret.add(fac.getOWLObjectSomeValuesFrom((OWLObjectPropertyExpression) candR,
              fac.getOWLThing()));
        }
      }
    }
  ...
  protected void retainOnlyExistentialRestrictions(Set<OWLClassExpression> cs) {
    ...
    for (Iterator<OWLClassExpression> it = cs.iterator(); it.hasNext();) {
      ...
      if (!c.getClassExpressionType().equals(ClassExpressionType.OBJECT_SOME_VALUES_FROM)
        ...
        it.remove();
        /*
         * c = aterm2complexAtermEq.get(c); if (c!=null && ATermUtils.isSomeValues(c)) { add.add(c);
         * }
         */
      }
  ...
  protected Set<OWLClassExpression> getCommunSubsumees(Set<OWLClassExpression> classes,
      boolean onlyExistialRestrictions) {
  ...
  @Override
  public Set<OWLPropertyExpression> getAllSubproperties(OWLPropertyExpression roleSup) {
    Set<OWLPropertyExpression> subs = new HashSet<OWLPropertyExpression>();
    for (OWLPropertyExpression sub : roleSup.isDataPropertyExpression()
        ? dataPropTaxo.getSubsumeesOrSelf(roleSup)
        : objPropTaxo.getSubsumeesOrSelf(roleSup)) {
      if (!sub.isAnonymous()) {
        subs.add(sub);
      } else {
        // OWLObjectProperty op = ((OWLObjectPropertyExpression) c).getNamedProperty();
        // //fac.getOWLObjectProperty(IRI.create(sub.getInverse().getName().getAFun().getName()));
        // if (oprops.contains(op))
        {
          subs.add(sub);
        }
        // else {
        // do nothing: inverse of a create role R, where R is defined as equivalent to the inverse
        // of an existing role
        // }
      }
    }
    ...
    subs.remove(
        fac.getOWLObjectInverseOf((OWLObjectProperty) objPropTaxo.getBottom()).getSimplified());
  ...
  public Set<OWLClassExpression> getUnsatisfiableClassExpressions() {
    ...
    Set<OWLClassExpression> ret =
        new HashSet<OWLClassExpression>(classTaxo.getBottomNode().getEquivalentElements());
    ret.remove(classTaxo.getBottom());
    return ret;
  }
  ...
  public Set<OWLPropertyExpression> getUnsatisfiablePropertyExpressions() {
    ...
    /*
     * for (OWLPropertyExpression c : objPropTaxo.getBottomNode().getEquivalentElements()) { if
     * (!c.isAnonymous()) { ret.add(c); } else { //OWLObjectProperty op =
     * ((OWLObjectPropertyExpression)
     * c).getNamedProperty();//fac.getOWLObjectProperty(IRI.create(ATermUtils.makeInv(c).getAFun().
     * getName())); //if (oprops.contains(op)) { ret.add(c); } //else { // do nothing: inverse of a
     * create role R, where R is defined as equivalent to the inverse of an existing role //} } }
     */
      ...
      for (OWLClassExpression c : mostGeneralSubmees) {
        ...
        for (OWLPropertyExpression subP : getAllSubproperties(p)) {
          if (subP.isObjectPropertyExpression()) {
            subsumees.add(tbox.getFactory().getOWLObjectSomeValuesFrom(
                (OWLObjectPropertyExpression) subP, tbox.getFactory().getOWLThing()));
          } else {
            assert subP.isDataPropertyExpression() : subP;
            subsumees.add(tbox.getFactory().getOWLDataSomeValuesFrom(
                (OWLDataPropertyExpression) subP, tbox.getFactory().getTopDatatype()));
  ...
  /**
   * returns a map associating a number of wildcards processed (>=
   * minimalNumberOfConceptWildcardsToReport) to the most general subsumees for the corresponding
   * number of wildcards. In other words, for k>= minimalNumberOfConceptWildcardsToReport),
  ...
  public Map<Integer, Set<OWLClassExpression>> computeMostGeneralSubsumees(
      Set<OWLClassExpression> classes, Set<OWLPropertyExpression> properties,
      int conceptWildcardsToProcess, int minimalNumberOfConceptWildcardsToReport) {
    if (minimalNumberOfConceptWildcardsToReport > conceptWildcardsToProcess) {
      throw new IllegalArgumentException(
          "minimalNumberOfConceptWildcardsToReport must be less than or equals to conceptWildcardsToProcess");
    }
    Map<Integer, Set<OWLClassExpression>> ret = new HashMap<Integer, Set<OWLClassExpression>>();
    computeMostGeneralSubsumees(classes, properties, conceptWildcardsToProcess,
        minimalNumberOfConceptWildcardsToReport, ret, 0);
    return ret;
  }

  protected void computeMostGeneralSubsumees(Set<OWLClassExpression> classes,
      Set<OWLPropertyExpression> properties, int conceptWildcardsToProcess,
      int minimalNumberOfConceptWildcardsToReport,
      Map<Integer, Set<OWLClassExpression>> numberOfWildcards2Results,
      int conceptWildcardsAlreadyProccessed) {
    ...
    if (conceptWildcardsToProcess <= 0) {
      if (!mandatoryPart.isEmpty()
          && conceptWildcardsAlreadyProccessed >= minimalNumberOfConceptWildcardsToReport) {
        Set<OWLClassExpression> res =
            numberOfWildcards2Results.get(conceptWildcardsAlreadyProccessed);
        if (res == null) {
      ...
      if (!classes.isEmpty() && !mandatoryPart.isEmpty()) {
        ...
        for (int i = 1; i <= conceptWildcardsToProcess; i++) {
          if (conceptWildcardsAlreadyProccessed + i >= minimalNumberOfConceptWildcardsToReport) {
            Set<OWLClassExpression> res =
                numberOfWildcards2Results.get(conceptWildcardsAlreadyProccessed + i);
            if (res == null) {
              res = new HashSet<OWLClassExpression>();
      ...
      Set<OWLClassExpression> remainingClasses =
          new HashSet<OWLClassExpression>(tbox.getNormalizedOntology().getClassesInSignature());
      ...
      for (OWLClassExpression c : remainingClasses) {
        ...
        computeMostGeneralSubsumees(newClasses, properties, conceptWildcardsToProcess - 1,
            minimalNumberOfConceptWildcardsToReport, numberOfWildcards2Results,
            conceptWildcardsAlreadyProccessed + 1);
  ...
  @Override
  public Set<OWLClassExpression> getMostGeneralSubsumees(Set<OWLClassExpression> classes,
      Set<OWLPropertyExpression> properties, int conceptPropertyWildcards, int propertyWildcards,
    ...
    if (conceptWildcards > 0) {
      ...
      for (OWLClassExpression c : remainingClasses) {
        Set<OWLClassExpression> newClasses = new HashSet<OWLClassExpression>(classes);
        newClasses.add(c);
        ret.addAll(getMostGeneralSubsumees(newClasses, properties, conceptPropertyWildcards,
            propertyWildcards, conceptWildcards - 1));
      ...
      // newPartialSubs.addAll(getMostGeneralSubsumees(Collections.singleton(c),
      ...
      // assert partialSub instanceof OWLQuantifiedRestriction;
      // OWLPropertyExpression p = ((OWLQuantifiedRestriction) partialSub).getProperty();
      // Set<OWLClassExpression> newClassesNotSeenYet = new
      // HashSet<OWLClassExpression>(classesNotSeenYet);
    ...
    if (conceptPropertyWildcards > 0 || propertyWildcards > 0) {
      if (ret.isEmpty()) {
        ...
        for (OWLDataProperty p : dprops) {
          ret.add(
              tbox.getFactory().getOWLDataSomeValuesFrom(p, tbox.getFactory().getTopDatatype()));
        }
