...
public class EliminateEJVar {
  ...
  protected static class Signature {
    ...
    public Signature(List<AtomicFormula> conceptWildcards,
        List<AtomicFormula> propertyWildcardsXisInFirstPos,
        List<AtomicFormula> propertyWildcardsXisInSecondPos, Rule r) {
      super();
      ...
      for (int i = 0; i < 2; i++) {
        for (AtomicFormula pw : i == 0 ? propertyWildcardsXisInFirstPos
            : propertyWildcardsXisInSecondPos) {
    ...
    public int hashCode() {
      ...
      result = prime * result + ((conceptWildcardsWithBoundVars == null) ? 0
      ...
      result = prime * result + ((propertyWildcardsWithBoundVars == null) ? 0
    ...
    public boolean equals(Object obj) {
      ...
      } else if (!conceptWildcardsWithBoundVars.equals(other.conceptWildcardsWithBoundVars)) {
        return false;
      }
      ...
      } else if (!propertyWildcardsWithBoundVars.equals(other.propertyWildcardsWithBoundVars)) {
        return false;
      }
      return true;
    }
    ...
    public String toString() {
      return "Signature [propertyWildcardXFirst=" + propertyWildcardXFirst
          + ", propertyWildcardXSecond=" + propertyWildcardXSecond + ", conceptWildcards="
          + conceptWildcards + ", conceptWildcardsWithHeadVars=" + conceptWildcardsWithBoundVars
          + ", propertyWildcardsWithHeadVars=" + propertyWildcardsWithBoundVars + "]";
    }
  ...
  RuleSystem eliminate(Rule r, VariableExpr x, int[] nextAvailableRuleId) {
    ...
    for (AtomicFormula af : r.getBody()) {
      ...
      if (!(af.getPredicate() instanceof DLAnnotatedPredicate) || af.getArity() == 0) {
        formulasWithoutEJVarX.add(af);
        continue;
      }
      DLAnnotatedPredicate dlp = (DLAnnotatedPredicate) af.getPredicate();
      // x is in af
      if (af.getArity() == 1 && x.equals(af.getArguments().get(0))) {
        ...
        if (x.equals(arg1) && x.equals(arg2)) {
          ...
          if (xIsAResource != null
              && xIsAResource.booleanValue() != prop.isObjectPropertyExpression()) {
            // x stand for a resource and a literal
            return null;
          }
    ...
    if (xIsAResource != null) {
      if (xIsAResource) {
        ...
        for (AtomicFormula af : binaryInSndPos) {
          DLAnnotatedPredicate dlp = (DLAnnotatedPredicate) af.getPredicate();
          OWLObjectPropertyExpression oprop =
              (OWLObjectPropertyExpression) dlp.getPropertyAnnotation();
          propexprs.add(oprop.getInverseProperty().getSimplified());
        }
      } else {
        for (AtomicFormula af : unary) {
          ExistInverseDataPropertyAnnotatedPredicate dlp =
        ...
        assert !propexprs.isEmpty(); // since at least one of the following sets is non-empty:
                                     // unary, binaryInFirstPos, binaryInSndPos.
      }
    } else {
    ...
    // }
    // }
    Map<Integer, Set<OWLClassExpression>> conceptWildcard2MGS =
        taxo.computeMostGeneralSubsumees(classexprs, propexprs,
            conceptWildcards.size() + conceptPropertyWildcards.size(), conceptWildcards.size());
    if (conceptWildcard2MGS.isEmpty() && (!classexprs.isEmpty() || !classexprs.isEmpty()
        || conceptWildcards.size() + conceptPropertyWildcards.size() > 0)) {
      return null;
    }

    Set<Rule> ret = new HashSet<Rule>();
    Set<Signature> signs = new HashSet<Signature>();
    int maxSignSize = 100000;// when the signature size is about this limit we disable the signature
                             // optimization
    for (Iterator<Pair<List<AtomicFormula>, List<AtomicFormula>>> it =
        getAllCombinationsOfConceptWildcardPropertyWildcard(conceptPropertyWildcards); it
            .hasNext();) {
      ...
      if (signs != null && !signs
          .add(new Signature(conceptWs, propertyWXisInFirstPos, propertyWXisInSecondPos, r))) {
      ...
      // mostGeneralSubsumees = taxo.getMostGeneralSubsumees(classexprs,
      // propexprs,0,propertyWs.size(),conceptWs.size());
      // more efficient approach than "mostGeneralSubsumees =
      // taxo.getMostGeneralSubsumees(classexprs, propexprs,0,propertyWs.size(),conceptWs.size());"
      ...
      if (binaryInFirstPos.isEmpty() && binaryInSndPos.isEmpty() && propertyWs.isEmpty()) {
          ...
          if (!conceptWs.isEmpty()) {
            ...
            for (AtomicFormula cw : conceptWs) {
              ...
              if (e1.isVariable() && !r.getUnboundVariables().contains(e1)) {
                ...
                AtomicFormula boundAf =
                    createBoundFormula(predicateVar, Collections.singleton(getRDFType()));
                newBody.add(0, boundAf);
              }
        ...
        for (OWLClassExpression ce : mostGeneralSubsumees) {
          ...
          List<AtomicFormula> newBody = new LinkedList<AtomicFormula>(
              replaceVariables(formulasWithoutEJVarX, allEqVars, newTerm));
          if (xIsAResource != null && !xIsAResource) {
            ...
            assert ce instanceof OWLDataSomeValuesFrom : ce; // since propexprs contains at least
                                                             // one data property
            //
            ...
            if (ce instanceof OWLObjectSomeValuesFrom) {
              ...
              OWLObjectSomeValuesFrom invC = fac.getOWLObjectSomeValuesFrom(
                  c.getProperty().getInverseProperty().getSimplified(), c.getFiller());
              ...
              newBodyPred = new ExistInverseDataPropertyAnnotatedPredicate(
                  (OWLDataProperty) dsv.getProperty(), fac);
            }
            AtomicFormula af = new AtomicFormula(newBodyPred, newTerm);
            newBody.add(0, af);
          }
   ...
   * @return
   */
  protected List<AtomicFormula> replaceVariables(List<AtomicFormula> afs,
      Set<VariableExpr> varsToReplace, Expr replacementExpression) {
   ...
   * @return
   */
  protected AtomicFormula replaceVariables(AtomicFormula af, Set<VariableExpr> varsToReplace,
      Expr replacementExpression) {
   ...
   * @return
   */
  protected Iterator<Pair<List<AtomicFormula>, List<AtomicFormula>>> getAllCombinationsOfConceptWildcardPropertyWildcard(
      List<AtomicFormula> conceptPropertyWildcards) {
    List<Pair<List<AtomicFormula>, List<AtomicFormula>>> results =
        new LinkedList<Pair<List<AtomicFormula>, List<AtomicFormula>>>();
    computeAllCombinationsOfConceptWildcardPropertyWildcard(conceptPropertyWildcards, results);
    return results.iterator();
  }

  private void computeAllCombinationsOfConceptWildcardPropertyWildcard(
      List<AtomicFormula> conceptPropertyWildcards,
      List<Pair<List<AtomicFormula>, List<AtomicFormula>>> results) {
    assert results != null;
    if (!conceptPropertyWildcards.isEmpty()) {
      AtomicFormula af = conceptPropertyWildcards.remove(0);
      List<Pair<List<AtomicFormula>, List<AtomicFormula>>> newresults =
          new LinkedList<Pair<List<AtomicFormula>, List<AtomicFormula>>>();
      ...
      results.add(Pair.make((List<AtomicFormula>) new LinkedList<AtomicFormula>(),
          (List<AtomicFormula>) new LinkedList<AtomicFormula>()));
    }
  }
      ...
      if (ex.getClassExpressionType().equals(ClassExpressionType.OBJECT_SOME_VALUES_FROM)
        ...
        if (!prop.isAnonymous()) {
          if (prop.isDataPropertyExpression()) {
            ...
            assert prop.isOWLObjectProperty() : prop;
  ...
  protected boolean isConceptPropertyWildcard(AtomicFormula af) {
    return af.getPredicate().getArity() == 3 && af.getArguments().get(1).isVariable()
        && af.getArguments().get(2).isVariable();
  }

  protected boolean isConceptWildcard(AtomicFormula af) {
    return af.getPredicate().getArity() == 3 && af.getArguments().get(1).isConstant()
        && RDF.type.getURI()
            .equals((((ConstantExpr) af.getArguments().get(1)).getValue().toString()))
        && af.getArguments().get(2).isVariable();
  }

  protected boolean isPropertyWildcard(AtomicFormula af) {
    Expr e;
    if (af.getPredicate().getArity() == 3 && af.getArguments().get(1).isVariable()
        && (e = af.getArguments().get(2)).isConstant()) {
      ConstantExpr ce = (ConstantExpr) e;
      if (ce.getValue() instanceof URI) {
        if (tbox.getNormalizedOntology().getClassesInSignature()
            .contains(tbox.getFactory().getOWLClass(IRI.create(ce.getValue().toString())))) {
          assert false : af; // assuming renaming of af to C(x) has already been done in the
                             // renaming step
          return false;
        } else {
          return true;
