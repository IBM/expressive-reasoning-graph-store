...
public class DefineAtomView {
  ...
  protected List<Rule> defineAtomView(DLAnnotatedPredicate ap, int[] nextAvailableRuleId) {
    ...
    if (ap instanceof ExistInverseDataPropertyAnnotatedPredicate) {
      ExistInverseDataPropertyAnnotatedPredicate eap =
          (ExistInverseDataPropertyAnnotatedPredicate) ap;
      OWLDataProperty prop = eap.getProperty();
      Set<OWLPropertyExpression> subs =
          new HashSet<OWLPropertyExpression>(taxo.getAllSubproperties(prop));
      subs.add(prop);
      for (OWLPropertyExpression sub : subs) {
        ...
        AtomicFormula head = ap.getArity() == 1 ? new AtomicFormula(ap, x)
            : new AtomicFormula(ap, Collections.EMPTY_LIST);
    ...
    if (classexp != null) {
      Set<OWLClassExpression> subs =
          new HashSet<OWLClassExpression>(taxo.getAllSubsumees(classexp));
      subs.add(classexp);
      for (OWLClassExpression sub : subs) {
        ...
        if (!sub.isAnonymous()) {
          ...
          body =
              new AtomicFormula(new Predicate(sub.asOWLClass().getIRI().toString(), 1), bodyVar1);
      ...
      if (pname.equals(OWL_DIFFERENTFROM_URI)) {
        ...
        subs.add(propexp);
        subs.add(fac.getOWLObjectInverseOf((OWLObjectProperty) propexp).getSimplified());
      } else {
      ...
      for (OWLPropertyExpression sub : subs) {
        ...
        AtomicFormula head =
            ap.getArity() == 2 ? new AtomicFormula(ap, x, y) : new AtomicFormula(ap);
        AtomicFormula body;
      ...
      if (propexp.isObjectPropertyExpression()) {
        ...
        if (taxo.getTbox().getReflexiveProperties().contains(op.getNamedProperty())) {
          ...
          // p(x,y) :- x = y;
          /*
           * VariableExpr x = new VariableExpr("x"); VariableExpr y = new VariableExpr("y");
           * AtomicFormula head = new AtomicFormula(ap, x, y); AtomicFormula body = new
           * AtomicFormula(new Predicate(Rule.BUILT_IN_EQUAL,2), x,y); ret.add(new Rule(head,
           * Collections.singletonList(body), nextAvailableRuleId[0]++));
           */
          ...
          try {
            ...
            body.add(new AtomicFormula(new Predicate(Rule.BUILT_IN_IRI_DIFF, 2), u,
                new ConstantExpr(new URI(RDF.type.getURI()))));
            body.add(new AtomicFormula(new BoundVariablePredicate(2), y, x));
            ret.add(new Rule(head, body, nextAvailableRuleId[0]++));
            //
  ...
  protected List<Rule> defineAtomViews(RuleSystem rs) {
    ...
    DLAnnotatedPredicate diffPred =
        new DLAnnotatedPredicate(fac.getOWLObjectProperty(IRI.create(OWL_DIFFERENTFROM_URI)));
  ...
  protected List<Rule> defineAtomViewForDifferentFrom(int ruleid) {
    ...
    for (OWLObjectProperty p : taxo.getTbox().getIrreflexiveProperties()) {
      ...
      AtomicFormula body = p.getSimplified().isAnonymous() ? new AtomicFormula(bodyPred, y, x)
          : new AtomicFormula(bodyPred, x, y);
    ...
    for (OWLDisjointObjectPropertiesAxiom disjprops : taxo.getTbox()
        .getNegativeObjectSubPropertyAxioms()) {
    ...
    for (OWLDisjointDataPropertiesAxiom disjprops : taxo.getTbox()
        .getNegativeDataSubPropertyAxioms()) {
      // add diff(x, y) : p(x, z) && q(y, z) (for q and p disjoint)
